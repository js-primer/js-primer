...
**対応する**。`Iterator.concat` は既存のIterator Helpers群と同じ粒度で紹介する。

### 追加位置の案

- **A: `[ES2025] イテレータのメソッド` 節内にサブ節として追加** (`Iterator.from` の後)
  - タイトル: `### [ES2026] Iterator.concat静的メソッド`
  - 既存のES2025メソッドと流れで学べるのでスムーズ
  - ES2025括りの中にES2026が混ざるのでタグ付けで対応
- **B: 独立した `[ES2026] Iterator.concat` 節として追加**
  - `[ES2025] イテレータのメソッド` 節の後ろに配置
  - 年次の独立で見た目は整う
  - 一方でIterator Helpers関連なのに節が離れるとつながりが弱くなる

Aがスムーズで章構成的にも自然。サブ節にES2026タグを付ければ問題ない。
...
- B案(独立節)にする場合は、Iterator Helpers関連であることを冒頭で明記する必要がある
