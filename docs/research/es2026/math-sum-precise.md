...
データ型とリテラル章
├── 数値（Number）
│   ├── 整数リテラル           (IEEE 754に言及、2^53-1の記述あり)
│   └── 浮動小数点数リテラル   (IEEE 754に言及するが精度ロスの話はなし)
├── [ES2020] BigInt
...
- 配列章 `Array.prototype.reduce` 節は「汎用reduceの教材」であり、合計は代表例として扱われているだけで「合計を求める」節ではない
- Math章にも「合計を求める」節はない
- 「合計を求める」という独立したテーマが本文に存在しないため、`Math.sumPrecise` を紹介する自然な足場がない
- 浮動小数点誤差の話題自体も入門者の典型課題ではなく、学習動機が弱い

Math章に単独で節を立てるのは収まりが悪い。「合計」や「浮動小数点誤差」のテーマが本文に整備されたら再検討の余地がある。
